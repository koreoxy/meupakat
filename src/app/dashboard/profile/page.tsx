...
import { getLevelInfoByXp, getXpProgress, LEVEL_BRACKETS, formatXp } from '@/lib/utils/xp';
...
export default function ProfilePage() {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 pt-6 pb-24 md:pb-10 space-y-4 animate-fade-in max-w-xl">

      {/* ── Header ─────────────────────────────────── */}
      <h1 className="font-display-md text-[var(--color-ink)] mb-2">Profile</h1>
        ...
        <div className="mt-5 pt-4 border-t border-[var(--color-hairline)]">
          ...
          <p className="text-[11px] text-[var(--color-ink-muted)] mt-1.5">
            {formatXp(levelInfo.xpNext - user.currentXp)} XP to {
              LEVEL_BRACKETS.find(b => b.xpRequired > user.currentXp)?.label ?? 'Max Level'
            }
          </p>
        </div>
      </Card>
      ...
      <Card variant="default">
        ...
        <div className="space-y-2">
          {LEVEL_BRACKETS.map((bracket) => {
            ...
            const isCurrent  = levelInfo.level === bracket.level;
            return (
              <div
                key={bracket.level}
                className={cn(
