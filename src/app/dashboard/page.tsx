...
import { getTodayProgressPercent, getStreakBadge } from '@/lib/utils/streak';
...
export default function DashboardPage() {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 pt-6 pb-24 md:pb-8 space-y-4 animate-fade-in max-w-2xl">
      ...
      <Card variant="feature">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            {isMissionDone ? '✅ Done!' : `${todayProgress?.minutesSpoken ?? 0}/${user.dailyTargetMinutes}m`}
          </span>
        </div>
        <ProgressBar
          value={todayPercent}
        ...
        {!isMissionDone && (
          <p className="text-[12px] text-[var(--color-ink-muted)] mt-2">
            {user.dailyTargetMinutes - (todayProgress?.minutesSpoken ?? 0)} minutes remaining today
          </p>
        )}
      </Card>
